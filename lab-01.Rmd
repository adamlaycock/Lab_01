...
visitors %>%
  group_by(region) %>%
    summarize(attraction_count = n_distinct(attraction)) %>%
...
NA stands for not available and it's because NA could be any value (unknown), so as a consequence the median and mean are also unknown.
...
Removes NA from the summary calculations.
...
There are a couple of attractions with very high visitor numbers compared to the rest of the dataset.
...
visitors_with_nations %>%
  group_by(nation) %>%
    summarise(n_attractions = n_distinct(attraction))
...
visitors_with_nations %>%
  group_by(nation) %>%
    summarise(free_admission = sum(admission == "Free") / n_distinct(attraction))
...
```{r Ex_n}
visitor_change <- visitors_with_nations %>%
  mutate(percentage_change = (((n_2022 - n_2021) / n_2021) * 100))

visitor_change %>%
  filter(nation == "Scotland") %>%
    group_by(admission) %>%
      summarise(percentage_change_visitors = mean(percentage_change, na.rm = TRUE)) %>%
        arrange(percentage_change_visitors)
```
